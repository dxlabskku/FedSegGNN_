...
class DisenVGSAN(nn.Module):
    ...
    def my_index_select_embedding(self, memory, index):
        ...
        index = index.view(-1)
        # Use weight directly to avoid CUDA kernel compatibility issues
    ...
    def graph_convolution(self, adj):
        # Detach adj to avoid gradient computation issues
    ...
    def add_position_embedding_s(self, seqs, seq_embeddings):
        ...
        seq_embeddings = seq_embeddings + position_embeddings  # Avoid inplace operation
    ...
    def forward(self, seqs, neg_seqs=None, aug_seqs=None):
        ...
        seqs_emb_s = seqs_emb_s * (self.item_emb_s.embedding_dim ** 0.5)  # Avoid inplace operation
        # (batch_size, seq_len, hidden_size)
        seqs_emb_e = seqs_emb_e * (self.item_emb_e.embedding_dim ** 0.5)  # Avoid inplace operation
        ...
        if self.training:
            ...
            # (batch_size, seq_len, hidden_size)
            neg_seqs_emb = neg_seqs_emb * (self.item_emb_e.embedding_dim ** 0.5)  # Avoid inplace operation
            # (batch_size, seq_len, hidden_size)
            aug_seqs_emb = aug_seqs_emb * (self.item_emb_e.embedding_dim ** 0.5)  # Avoid inplace operation
