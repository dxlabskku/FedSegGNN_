...
def save_config(args, verbose=True):
    ...
    with open(config_filename, "w") as outfile:
        # Some argparse fields (e.g., torch.device) are not JSON serializable;
        # fall back to string for unsupported types.
        json.dump(args_dict, outfile, indent=2, default=str)
    if verbose:
