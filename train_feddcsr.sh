...
# ============================================
# FedDCSR Training Script
# ============================================
# This script trains the FedDCSR model with configurable parameters
...
# Options: FedDCSR (ours), FedVGSAN, LocalVGSAN, FedSASRec, LocalSASRec,
#          FedVSAN, LocalVSAN, FedContrastVAE, LocalContrastVAE,
#          FedCL4SRec, LocalCL4SRec, FedDuoRec, LocalDuoRec
METHOD=FedDCSR
...
# Optional: toggle individual FedDCSR loss terms (default: on)
DCSR_DISABLE_SIM=false          # hinge similarity loss z_s vs z_g
DCSR_DISABLE_EXCLUSIVE=false    # exclusive reconstruction loss from z_e
DCSR_DISABLE_CONTRASTIVE=false  # contrastive loss between z_e and aug_z_e
DCSR_DISABLE_KLD=false          # KL divergence on z_s/z_e
...
python -u main.py \
    ...
    --mu $MU \
    --gpu $GPU \
    $( is_true "$DCSR_DISABLE_SIM" && echo "--dcsr_disable_sim" ) \
    $( is_true "$DCSR_DISABLE_EXCLUSIVE" && echo "--dcsr_disable_exclusive" ) \
    $( is_true "$DCSR_DISABLE_CONTRASTIVE" && echo "--dcsr_disable_contrastive" ) \
    $( is_true "$DCSR_DISABLE_KLD" && echo "--dcsr_disable_kld" ) \
    $LOAD_PREP \
    $DOMAINS
